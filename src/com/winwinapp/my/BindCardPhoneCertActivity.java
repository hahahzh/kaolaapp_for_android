	...
	private Handler mHandler = new Handler(){
		public void handleMessage(Message msg){
			...
			case 2:
				...
				if("OK".equals(error)){
					Toast.makeText(BindCardPhoneCertActivity.this, "短信验证码发送成功,请输入验证码", Toast.LENGTH_LONG).show();
				}else{
					Toast.makeText(BindCardPhoneCertActivity.this, "短信验证码发送失败："+error, Toast.LENGTH_LONG).show();
				}
				break;
			case 3:
				error = (String)msg.obj;
				if("OK".equals(error)){
					Toast.makeText(BindCardPhoneCertActivity.this, "绑定银行卡成功", Toast.LENGTH_LONG).show();
					Intent intent = new Intent(BindCardPhoneCertActivity.this,KLHomePageActivity.class);
					intent.putExtra("page", 3);
					intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
					startActivity(intent);
				}else{
					Toast.makeText(BindCardPhoneCertActivity.this, "绑定银行卡失败："+error, Toast.LENGTH_LONG).show();
				}
				break;
			}
		}
